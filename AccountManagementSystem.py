...
# FUNCTION BANAM HAI TA SURUMA

def Database():
    global conn, cursor
    conn = sqlite3.connect('account.db')
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, firstname TEXT, lastname TEXT,"
                   " gender TEXT, address TEXT, username TEXT, password TEXT)")
...
def Delete():
    if not tree.selection():
       txt_result.config(text="Please select an item first", fg="red")
        ...
        result = tkMessageBox.askquestion('Account Management System', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
...
# LABEL WIDGETS ho hai

txt_title = Label(Top, width=900, font=('Calibri', 24), text = "Account Management System",  bg='#ff5733', fg='#a9fc30')
txt_title.pack()
...
btn_read = Button(Buttons, width=10, text="View", command=Read )
btn_read.pack(side=LEFT)
btn_update = Button(Buttons, width=10, text="Update", command=Update, state=DISABLED)
btn_update.pack(side=LEFT)
btn_delete = Button(Buttons, width=10, text="Delete", command=Delete, bg='red')
